...
func (rs *Responder) InternalServerError(w http.ResponseWriter, r *http.Request, err error) {
	...
	_ = WriteJSONError(w, http.StatusInternalServerError, "the server encountered a problem", reqID(r))
}

func (rs *Responder) Forbidden(w http.ResponseWriter, r *http.Request, err error) {
	msg := "forbidden"
	if err != nil {
		msg = err.Error()
	}
	rs.Logger.Warnw("forbidden", "request_id", reqID(r), "method", r.Method, "path", r.URL.Path, "error", msg)
	_ = WriteJSONError(w, http.StatusForbidden, msg, reqID(r))
}
...
func (rs *Responder) BadRequest(w http.ResponseWriter, r *http.Request, err error) {
	...
	_ = WriteJSONError(w, http.StatusBadRequest, err.Error(), reqID(r))
}
...
func (rs *Responder) Conflict(w http.ResponseWriter, r *http.Request, err error) {
	...
	_ = WriteJSONError(w, http.StatusConflict, err.Error(), reqID(r))
}
...
func (rs *Responder) NotFound(w http.ResponseWriter, r *http.Request, err error) {
	...
	_ = WriteJSONError(w, http.StatusNotFound, "not found", reqID(r))
}
...
func (rs *Responder) RateLimitExceeded(w http.ResponseWriter, r *http.Request, retryAfter string) {
	...
	_ = WriteJSONError(w, http.StatusTooManyRequests, "rate limit exceeded, retry after: "+retryAfter, reqID(r))
}
